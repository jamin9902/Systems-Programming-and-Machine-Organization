...
void ball_thread(pong_ball* b) {
    ...
    while (true) {
        ...
        std::unique_lock<std::mutex> g_ball (main_board -> m_rows[b -> y + 1]);
        // Block if stopped
        if(b -> stopped) {
            b -> block.wait(g_ball);
        }
...
void warp_thread(pong_warp* w) {
    ...
    while (true) {
        std::unique_lock<std::mutex> g_warp (w -> m_warp);
        // Block until balls in ball queue
        while (w -> ball_queue.empty()) {
            w -> block_warp.wait(g_warp);
        }
        // Take first ball from ball queue and pop
        ...
        std::unique_lock<std::mutex> g_ball (main_board -> m_rows[w -> y + 1]);
