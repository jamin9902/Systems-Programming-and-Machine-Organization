...
void ball_thread(pong_ball* b) {
    ...
    while (true) {
        ...
        std::unique_lock<std::mutex> guard (main_board -> m_rows[b -> y + 1]);
        // Block if stopped
        if (b -> stopped) {
            b -> block.wait(guard);
        }
    }
    main_board -> m_rows[b -> y].unlock();
    delete b;
    --nrunning;
}
...
void warp_thread(pong_warp* w) {
    ...
    while (true) {
        std::unique_lock<std::mutex> w_guard(w -> m_warp);
        // Block while waiting for ball queue is empty
        while (w -> ball_queue.empty()) {
            w -> block_warp.wait(w_guard);
        }
        // Take first ball and pop from queue
        pong_ball* b = w -> ball_queue.front();
        w -> ball_queue.pop();
        // Avoid deadlock
        w_guard.unlock();
        ...
        std::unique_lock<std::mutex> b_guard(main_board -> m_rows[b -> y + 1]);
