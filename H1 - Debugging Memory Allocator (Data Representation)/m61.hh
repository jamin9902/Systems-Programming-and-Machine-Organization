...
#include <new>
#include <random>
#define END ((char*) 0x15FFFFF)
#define ALLOCATED ((char*) 0x15FFF00)
#define FREE ((char*) 0x1E63F00)
...
struct metadata {
    metadata* prev;           // pointer to previous node
    metadata* next;           // pointer to next node
    char* plptr;              // pointer to payload
    char* state;              // state of allocation: free (1) or allocated (2)
    size_t size;              // size of allocation
    const char* file;         // file from allocation
    long line;                // line from allocation
    long allign;              // added for allignment
};

struct boundary {
    char* buffer;             // buffer past end of memory block (1) if not overwritten
};

struct heavyhitter {
  heavyhitter* next;          // pointer to previous node
	heavyhitter* prev;          // pointer to next node
	size_t size;                // size of allocation
	const char* file;           // file from allocation
	long line;                  // line from allocation
};
